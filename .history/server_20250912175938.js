...
app.use(
  expressSession({
    ...
    store: MongoStore.create({ mongorl: process.env.MONGODB_URL }), //specifying where the cookie will be stored(that's the database)
...
//passport configs
app.use(passport.initialize()); //initializing passport.js. it has many duties like looks for login attempts
app.use(passport.session()); //using passport.session..which is an inbuilt function

//aunthentication with passport local strategy
passport.use(UserModel.createStrategy()); //
passport.serializeUser(UserModel.serializeUser()); //when login happens it embeds the person into the system during the session(given an id Number)
passport.deserializeUser(UserModel.deserializeUser()); //when logout occurs the person is disembedded(the number is destroyed)
