...
const item = await stockoutModel.findById(req.params.id)
  res.render(`editstock`, {id : req.params.id});
})
...
router.put("/editstock/:id", async (req, res) => {
  try {
    ...
    const item = await stockoutModel.findById(req.params.id, req.body, { new: true });
