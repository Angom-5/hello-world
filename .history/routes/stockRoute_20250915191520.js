...
router.post("/stockOutForm", async (req, res) => {  //the stockoutList is just a name
  try {
    let itemss = await stockoutModel.find().sort({ $natural: -1 });
    console.log(items);
...
router.put("/editstock/:id", async (req, res) => {
  try {
    const stockout = await stockoutModel.findById(req.params.id);
    stockout.itemname = req.body.itemname;
    stockout.quantity = req.body.quantity;
    stockout.category = req.body.category;
    stockout.save();
