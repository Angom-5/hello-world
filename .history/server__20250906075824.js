...
app.use('/about', (req, res, next) => {
  ...
  next(); // this function tells that more processing is needed for the curren request and is in the next route handler/middleware function
});


//third party middleware
//allows developers to reuse code in their work as well as share it
//eg; bodyParser
//allows developers process incoming data

//cookieParser
//it parses Cookie header and populate req.cookies with an object keyed by cookie names.

//serving static files
app.use(express.static('public'));// specify where our server files are
//public is the folder which holds them
...
//MVC
//Creates an interconnection between parts.
//code re-use and parallel development..meaning two people can be working on different things on the project but at the same time


//if you have a nav bar(or any feature) which is used by more than one file you need to pull it out and and create a layout
//see pug files; layout and stockin
//you place block content after the shared thing's code
//then for the other files which share that same feature;
//block content is placed where the 

//full path in routing
app.use("/class", stockRoute);
// if your code has a full path it means that the directory to access the files should start with /class
 
//eg;
//routes
router.get("/class/stockOutForm", (req, res) => {
  res.render("stock-outForm");
});

router.post("/class/stockOutForm", (req, res) => {
  console.log(req.body);
  res.redirect('/class/registerStock')
});

//even on the main element in the action attribute, it should have the file directory starting with /class
