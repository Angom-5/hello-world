...
passport.use(serModel.createStrategy());  //
passport.serializeUser(serModel.serializeUser());  //when login happens it embeds the person into the system during the session(given an id Number)
passport.deserializeUser(serModel.deserializeUser());  //when logout occurs the person is disembedded(the number is destroyed)
